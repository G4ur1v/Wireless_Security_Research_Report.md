...
2. The study focuses on the 4-Way Handshake vulnerability and the execution of a Man-in-the-Middle (MitM) attack via an Evil Twin Access Point. Uniquely, this research utilized an ESP32 microcontroller as the target Access Point to simulate a portable IoT environment. All tests were performed in a controlled lab environment on self-owned hardware.
...
The ESP32 AP was continuously jammed, forcing the victim to migrate to the Evil Twin. The credentials entered by the user were captured and instantly verified against the previously captured handshake.
...
8. Conclusion
The research successfully demonstrated that WPA2 vulnerabilities persist across all hardware scales—from high-end routers to small ESP32 microcontrollers. The integration of specialized chipsets and Kali Linux is essential for identifying these flaws.
